...
def train_test_split(path, timesteps, testing):
    ...
    x_data, y_data = create_sequences(data[['accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z']], data['activity'], timesteps,
                                      unique_activities)
...
def preprocessing_data(X_train_augmented, y_train_augmented, X_test, y_test):
    scaler = RobustScaler()
    #Accel data
    X_train_acc = X_train_augmented[:, :, :3]
    X_train_flat = X_train_acc.reshape(-1, X_train_acc.shape[-1])
    X_train_flat = scaler.fit_transform(X_train_flat)
    X_train_acc = X_train_flat.reshape(X_train_acc.shape)

    X_test_acc = X_test[:, :, :3]
    X_test_flat = X_test_acc.reshape(-1, X_test_acc.shape[-1])
    X_test_flat = scaler.transform(X_test_flat)
    X_test_acc = X_test_flat.reshape(X_test_acc.shape)

    # Gyro data
    X_train_gyro = X_train_augmented[:, :, 3:]
    X_train_flat = X_train_gyro.reshape(-1, X_train_gyro.shape[-1])
    X_train_flat = scaler.fit_transform(X_train_flat)
    X_train_gyro = X_train_flat.reshape(X_train_gyro.shape)

    X_test_gyro = X_test[:, :, 3:]
    X_test_flat = X_test_gyro.reshape(-1, X_test_gyro.shape[-1])
    X_test_flat = scaler.transform(X_test_flat)
    X_test_gyro = X_test_flat.reshape(X_test_gyro.shape)

    # Combine them
    X_train_augmented = np.hstack((X_train_acc, X_train_gyro))
    X_test = np.hstack((X_test_acc, X_test_gyro))
...
def train_sequential_model(X_train, y_train, X_test, y_test, chosen_model, class_labels, filename, train_model):
    ...
    file_name = f'files_{filename}/saved_models_{filename}/comb_{chosen_model}_model.h5'
...
def plot_confusion_matrix(y_test_labels, y_pred_labels, smoothed_predictions, class_labels, chosen_model, filename):
    ...
    for norm_value in normalize_cm:
        if norm_value == 'true':
            format = '.2f'
            plot_name = f'comb_{chosen_model}_cm_norm_inital.png'
        else:
            format = 'd'
            plot_name = f'comb_{chosen_model}_cm_initial.png'
    ...
    for norm_value in normalize_cm:
        if norm_value == 'true':
            format = '.2f'
            plot_name = f'comb_{chosen_model}_cm_norm_smooth.png'
        else:
            format = 'd'
            plot_name = f'comb_{chosen_model}_cm_smooth.png'
