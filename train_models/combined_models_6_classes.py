...
def process_data(path, timesteps, testing):
    ...
    x_data, y_data = create_sequences(data[['accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z']], data['activity'], timesteps, unique_activities)
    print(x_data.shape)
    ...
    reshaped_data = filtered_windows.reshape(-1, 6)
    if not testing:
        scaler = RobustScaler()
        scaled_data = scaler.fit_transform(reshaped_data)
        final_x_data = scaled_data.reshape(len(x_data), timesteps, 6)
...
def plot_confusion_matrix_grouped(y_test_labels, y_pred_labels, smoothed_predictions, class_labels, chosen_model, filename):
    ...
    for norm_value in normalize_cm:
        if norm_value == 'true':
            format = '.2f'
            plot_name = f'comb_{chosen_model}_cm_norm_inital_grouped.png'
        else:
            format = 'd'
            plot_name = f'comb_{chosen_model}_cm_initial_grouped.png'
    ...
    for norm_value in normalize_cm:
        if norm_value == 'true':
            format = '.2f'
            plot_name = f'comb_{chosen_model}_cm_norm_smooth_grouped.png'
        else:
            format = 'd'
            plot_name = f'comb_{chosen_model}_cm_smooth_grouped.png'
...
if __name__ == '__main__':
    ...
    class_labels = ['cycling', 'exercising', 'running', 'sitting', 'standing', 'walking']
