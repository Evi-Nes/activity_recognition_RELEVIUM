...
for folder in tqdm(os.listdir(main_folder)):
    ...
    if os.path.isdir(folder_path) and folder.startswith('user_'):
        ...
        # sleep_path = os.path.join(folder_path, 'smartwatch_gyr.csv')
        ...
        if os.path.exists(activity_labels_path):
            ...
            activity_labels = activity_labels[activity_labels['Day'] != 2]
            smartwatch_acc = smartwatch_acc[smartwatch_acc['day'] != 2]
            ...
            # columns_to_check = ['accel_x', 'accel_y', 'accel_z']
            # result = result[(result[columns_to_check] != 0).any(axis=1)]
...
if merged_files:
    ...
    if all_data:
        ...
        combined_file_path = os.path.join(main_folder, 'final_multilevel.csv')
        combined_df_cleaned.to_csv('final_multilevel.csv', index=False)
